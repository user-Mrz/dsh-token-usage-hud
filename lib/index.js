...
/** Beijing calendar date key (YYYY-MM-DD) for one instant. */
export function beijingDateKey(date) {
	...
	try {
		...
		if (year !== undefined && month !== undefined && day !== undefined) return `${year}-${month}-${day}`;
	} catch {
		/* fall through to the UTC+8 shift */
	}
	return new Date(instant.getTime() + 8 * 3600e3).toISOString().slice(0, 10);
}
 ...
 * holidays (which bill off-peak all day, like weekends). Everything else is
 * off-peak. Statutory holidays come from `offpeakDates` (`YYYY-MM-DD` Beijing
 * dates) — the harness ships no holiday calendar, so this stays opt-in.
 * @param date - instant to classify.
 * @param offpeakDates - Beijing date keys treated as all-day off-peak.
 */
export function isPeakHour(date, offpeakDates) {
	const instant = date instanceof Date ? date : new Date(date);
	if (Array.isArray(offpeakDates) && offpeakDates.length > 0 && offpeakDates.includes(beijingDateKey(instant))) return false;
	let weekday;
	let hour;
	try {
...
export function createFold(offpeakDates = []) {
	return {
		consumed: 0,
		model: undefined,
		provider: undefined,
		...
		steps: 0,
		offpeakDates: Array.isArray(offpeakDates) ? offpeakDates : []
	};
}
	...
	switch (event.type) {
		...
		case "assistant/message": {
			...
			if (usage !== undefined && usage !== null && typeof usage === "object") {
				...
				const period = isPeakHour(event.time ?? Date.now(), fold.offpeakDates) ? "peak" : "offpeak";
...
export function finalizeFold(fold, prices, currency, now = new Date()) {
	...
	return {
		...
		period: isPeakHour(now, fold.offpeakDates) ? "peak" : "offpeak",
...
export function resolveConfig(config) {
	...
	const balance = {
		...
		refreshMs: typeof rawBalance.refreshMs === "number" && Number.isFinite(rawBalance.refreshMs) && rawBalance.refreshMs >= 5000 ? Math.round(rawBalance.refreshMs) : 15000,
	...
	// 活动期重拉下限：即使被失效标记，两次余额请求之间也至少间隔这么久
	// （对话流式期间事件密集，避免把 DeepSeek 余额接口打成每分钟几十次）。
	const floorRaw = typeof rawBalance.refetchFloorMs === "number" && Number.isFinite(rawBalance.refetchFloorMs) && rawBalance.refetchFloorMs >= 1000 ? Math.round(rawBalance.refetchFloorMs) : 5000;
	balance.refetchFloorMs = Math.min(floorRaw, balance.refreshMs);
	// 中国法定节假日（北京时间 YYYY-MM-DD）：全天按空闲价计费。
	const offpeakDates = Array.isArray(cfg.offpeakDates)
		? cfg.offpeakDates.filter((entry) => typeof entry === "string" && /^\d{4}-\d{2}-\d{2}$/u.test(entry))
		: [];
	return { enabled: cfg.enabled !== false, currency, position, pollMs, visible, prices, balance, offpeakDates };
}
...
async function fetchBalance(ctx, options) {
	try {
		...
		// 优先取与配置币种一致的条目（避免多币种账户拿错第一条）。
		const want = options.currency === "USD" ? "USD" : "CNY";
		const info = infos.find((entry) => entry !== null && typeof entry === "object" && entry.currency === want)
			?? infos.find((entry) => entry !== null && typeof entry === "object" && typeof entry.currency === "string")
			?? infos[0];
		return {
...
/**
 * Cached balance reader. Freshness: a successful value is served for
 * `balance.refreshMs`, or until a usage event invalidates it (`invalidatedAt`
 * after the last fetch) — in which case the next poll refetches promptly,
 * bounded by `balance.refetchFloorMs` so heavy streaming cannot hammer the
 * balance API. One in-flight promise is shared (no stampede); on failure the
 * previous value is kept and the retry backs off for `refreshMs`.
 */
async function getBalanceCached(ctx, options, cache) {
	const now = Date.now();
	const sinceFetch = now - cache.fetchedAt;
	const fresh = cache.value !== undefined
		&& cache.value.error === undefined
		&& sinceFetch < options.balance.refreshMs
		&& (cache.invalidatedAt ?? 0) <= cache.fetchedAt;
	if (fresh) return cache.value;
	// 活动期防抖下限：刚刷过（未满 refetchFloorMs）先返回旧值，下一轮再拉。
	if (cache.value !== undefined && cache.value.error === undefined && sinceFetch < options.balance.refetchFloorMs) {
		return cache.value;
	}
...
const apply = mountOnce(name, (ctx, config) => {
	...
	const balanceCache = { value: undefined, fetchedAt: 0, inflight: undefined, invalidatedAt: 0 };
	const upsert = (session) => {
		let state = states.get(session.id);
		if (state === undefined) {
			state = { fold: createFold(options.offpeakDates), session };
	...
	const offEvent = ctx.on("session/event", (session, event) => {
		...
		try {
			...
			upsert(session);
			// 有新的计费用量落地 -> 余额缓存失效，下一次轮询即重新拉取，
			// 让余额随对话消耗实时同步（受 refetchFloorMs 防抖下限约束）。
			if (options.balance.enabled && event !== undefined && event !== null
				&& event.type === "assistant/message" && event.data?.usage !== undefined) {
				balanceCache.invalidatedAt = Date.now();
			}
		} catch (error) {
	...
	const route = ctx.webServer.register({
		...
		handler: async (req, res) => {
			// 桌面端（Electron/WebView 等以自定义协议加载界面）会从页面来源跨源
			// 访问本机 host：回环请求开放 CORS（含预检），非回环仍然 403。
			...
			if (sessionId === "") {
				writeJson(res, 400, { ok: false, error: "missing session id" }, cors);
				return;
			}
			let session;
			try {
			...
			if (session === undefined) {
				writeJson(res, 200, { ok: true, sessionId, exists: false }, cors);
				return;
			}
			try {
				...
				writeJson(res, 500, { ok: false, error: String(error?.message ?? error) }, cors);
			}
		}
	});
	...
	try {
		...
		const usedTokens = typeof pressure.projectedTokens === "number"
			...
			: typeof pressure.pressureTokens === "number"
				...
				: null;
		const model = fold.model ?? "unknown";
		const period = isPeakHour(new Date(), fold.offpeakDates) ? "peak" : "offpeak";
...
/**
 * Loopback fence. The socket and the Host header must both be loopback (that
 * is what keeps a remote client out), and a browser page may only reach it
 * same-origin. Desktop shells load the UI from a custom scheme (`app://`,
 * `file://` → Origin "null") and fetch the local host cross-origin, so a
 * non-http(s) origin is accepted as local: a remote web page cannot present
 * one, and it still has to pass the loopback socket + host checks.
 */
function isLoopbackRequest(request) {
	...
	// 自定义协议（桌面端本地壳）：来源不可能是远程站点，socket/host 已确认为回环。
	return true;
}

/** CORS headers echoing the caller's Origin (loopback-fenced callers only). */
function corsHeadersFor(request) {
	...
	if (typeof origin === "string" && origin !== "" && origin !== "null") headers["access-control-allow-origin"] = origin;
	return headers;
}
