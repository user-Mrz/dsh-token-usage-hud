...
export function resolveConfig(config) {
	...
	return { enabled: cfg.enabled !== false, currency, position, pollMs, visible, prices };
}
		...
		handler: async (req, res) => {
			...
			try {
				...
				writeJson(res, 200, {
					...
					context: readContextProjection(ctx, session, state.fold, options.prices, options.currency)
				});
			} catch (error) {
