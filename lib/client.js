window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 桌面端（无 http location / 自定义协议）回退的 host 地址。 */
		...
		function resolveClientConfig(config) {
			...
			return {
				...
				hostURL: typeof cfg.hostURL === "string" ? cfg.hostURL.trim() : ""
			};
		}

		/**
		 * API base candidates, most-preferred first — the same resolution order the
		 * DSH client itself uses (`location.origin` when the page has a real origin),
		 * extended for desktop shells:
		 *   1. explicit `hostURL` config (wins; use it when the host runs off-page);
		 *   2. the page origin (browser and any webview served by the host — same
		 *      origin, and a shell's transport patch can rewrite the path);
		 *   3. `http://127.0.0.1:3080` (custom-scheme / no-location desktop shells).
		 * `location.origin` can be "null" (file://) or absent (Node-like shells),
		 * in which case only 1 and 3 remain.
		 */
		function apiBaseCandidates(config) {
			const list = [];
			const push = (value) => {
			...
			if (typeof origin === "string" && origin !== "" && origin !== "null") push(origin);
			push(DEFAULT_HOST_URL);
			return list;
		}
		...
		function apply(ctx, config) {
			...
			ctx.effect(() => {
				...
				let lastSteps = -1;
				let disposed = false;
				// API base negotiation: the first candidate that answers is remembered
				// and tried first afterwards; a failure re-scans the list (web server
				// moved, desktop shell switched transport, host started later).
				...
				(() => {
					const css = [
						...
						"[data-token-usage-hud=\"box\"] [data-token-usage-hud=\"bal\"]{color:var(--dsw-alias-state-success-primary, #37d67a);font-weight:600}",
						"[data-token-usage-hud=\"box\"][data-bal-error=\"true\"] [data-token-usage-hud=\"bal\"]{color:var(--dsw-alias-state-warn-primary, #f5b85c)}",
				...
				const render = (stats) => {
					...
					// 账户剩余金额（host 端从 DeepSeek /user/balance 读取，仅回传数字）
					const bal = stats.balance;
					if (bal !== null && typeof bal === "object") {
						if (bal.error !== undefined) {
							lines.push("余额 <span data-token-usage-hud=\"bal\">获取失败</span>");
							root.setAttribute("data-bal-error", "true");
						} else if (typeof bal.total === "number") {
							const currency = typeof bal.currency === "string" ? bal.currency : options.currency;
							const amount = fmtMoney(bal.total, currency);
							const breakdown = [
								typeof bal.toppedUp === "number" && bal.toppedUp > 0 ? "充值 " + fmtMoney(bal.toppedUp, currency) : "",
								typeof bal.granted === "number" && bal.granted > 0 ? "赠送 " + fmtMoney(bal.granted, currency) : ""
							].filter(Boolean).join(" · ");
							lines.push("余额 <span data-token-usage-hud=\"bal\">" + amount + "</span>" + (breakdown ? "  <dim title=\"" + breakdown + "\">" + (bal.isAvailable === false ? "不可用" : "") + "</dim>" : ""));
							root.removeAttribute("data-bal-error");
						}
					}
				...
				/** One fetch attempt against a specific base; null on any failure. */
				const fetchStats = async (base) => {
					try {
						const response = await fetch(base + API_PATH + "?session=" + encodeURIComponent(currentId), { cache: "no-store" });
				...
				const poll = async () => {
					...
					let stats = null;
					// 先试上次成功的 base（Web 端即页面同源；桌面端可能是回退的 host 地址）。
					if (activeBase !== null) {
						stats = await fetchStats(activeBase);
						if (stats === null) activeBase = null;
					}
					if (stats === null) {
						for (const base of bases) {
							if (disposed) return;
							const candidate = await fetchStats(base);
							if (candidate !== null) {
								stats = candidate;
								activeBase = base;
								break;
